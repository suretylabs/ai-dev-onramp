...
flowchart TB
    ...
    subgraph python_stack["1. Python project stack"]
        direction LR
        p1["1. Python 3.14<br/>interpreter"] --> p2["2. Project environment<br/>(.venv)"] --> p3["3. Installed dependencies"] --> p4["4. Application code"]
    end
    ...
    subgraph uv_control["2. uv is the control plane"]
        ...
        subgraph uv_first["Provision and declare"]
            direction LR
            u1["1. Install Python"] --> u2["2. Create the project environment"] --> u3["3. Declare and install dependencies"]
        end
        subgraph uv_second["Reproduce and run"]
            direction LR
            u4["4. Lock exact versions"] --> u5["5. Run commands in project context"]
        end
        u3 --> u4
    end
    ...
    subgraph dependency_state["3. Dependency state"]
        ...
        subgraph dependency_first["Declared intent and resolution"]
            direction LR
            dep1["pyproject.toml declares project intent"] --> dep2["uv sync resolves and installs"]
        end
        subgraph dependency_second["Isolation and reproducibility"]
            direction LR
            dep3[".venv holds the isolated environment"] --> dep4["uv.lock records exact resolved versions"]
        end
        dep2 --> dep3
    end
    ...
    subgraph data_stack["4. Data-stack mental model"]
        ...
        subgraph data_first["Ingest and transform"]
            direction LR
            d1["1. MSSQL, PDFs, and source files"] --> d2["2. Python orchestration"] --> d3["3. Polars transforms data"]
        end
        subgraph data_second["Store and consume"]
            direction LR
            d4["4. Parquet stores analytical data"] --> d5["5. DuckDB queries with SQL"] --> d6["6. Validation, reporting, and exports"]
        end
        d3 --> d4
    end
    ...
    subgraph proof["5. First integrated proof"]
        ...
        subgraph proof_first["Build the artifact"]
            direction LR
            q1["1. Create a small representative input"] --> q2["2. Load and transform with Polars"] --> q3["3. Write Parquet"]
        end
        subgraph proof_second["Query and validate"]
            direction LR
            q4["4. Query with DuckDB"] --> q5["5. Validate the expected output"] --> q6["6. Run Ruff, Pyright, and pytest"]
        end
        q3 --> q4
    end
